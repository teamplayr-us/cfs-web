// Render the ISI-branded IG "team accepted" announcement.
//
...
//   2. Logo path relative to the repo root (transparent PNG), e.g.
//      public/programs/foo.png or collateral/invites/foo.png
...
if (!name || !logoFile || !location || !divisions) {
  console.error(
    'Usage: node render-isi-announce.mjs "Program Name" logo.png "City/Region" "12U · 14U" ["Event line"] [City] [out.png]',
  );
  process.exit(1);
}

const html = readFileSync(join(here, "ig-org-accepted-isi.html"), "utf8")
  .replaceAll("{{NAME}}", name)
  .replaceAll("{{LOGO}}", relative(here, resolve(process.cwd(), logoFile)))
...
const tmp = join(here, ".ig-org-accepted-isi.tmp.html");
writeFileSync(tmp, html);

const out = join(
  here,
  outFile || `ig-isi-${basename(logoFile).replace(/\.(png|jpg|jpeg)$/i, "")}.png`,
);
